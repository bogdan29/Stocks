...
namespace Stocks
{
    public class Startup
    {
        ...
        {
            services.AddTransient<IStockDataDispatcher, StocksDataDispatcher>(sp => new StocksDataDispatcher(Configuration.GetValue<string>("AlphavantageApiKey")));
            services.AddTransient<IStockDataRepository, StockDataRepository>();
            services.AddTransient<MongoDbContext>(sp => new MongoDbContext(
                Configuration.GetValue<string>("MongoDbConnection"),
                Configuration.GetValue<string>("MongoDbDatabase")));
            services.AddMediatR(typeof(StockQuery).Assembly);
